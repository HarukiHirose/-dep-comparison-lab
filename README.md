...
4. **コード量/保守性**: LOC、依存パッケージ数(直接+推移的)
5. **実行時の挙動(E2Eテスト)**: `metrics/e2e-test.mjs` が両アプリを実際に起動し、
   register→login→タスク作成→一覧→更新→削除まで本物のHTTPリクエストで通しで検証する。
   `npm audit` や `tsc` のような静的チェックでは検出できない「実際に動かして初めて分かる問題」
   (依存の更新によるレスポンス形式の変化、実行時エラー、起動時間、メモリ使用量など)を捉えるのが狙い。
   毎日の自動収集にも組み込まれており、記録される値は以下の通り:
   - `allPassed`: 一連の操作が全て仕様通り成功したか
   - `bootMs`: サーバー起動〜疎通確立までの時間
   - `memoryKb`: 起動直後のメモリ使用量(RSS, Linux環境のみ計測可)
   - `avgStepMs`: 各APIリクエストの平均レスポンスタイム
   - `failedSteps`: 失敗したステップ名の一覧(あれば)
